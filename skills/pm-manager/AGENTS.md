...
1. 优先日常五条：`/pm-init` `/pm-status` `/pm-next` `/pm-done` `/pm-fix`；发布前 `/pm-all`。模块深潜走 `/pm-all`，不要让用户跑 `/pm-discover`。
...
8. 评审走 `/pm-review`：推理链 + 引用片段；`confirm` / `false_positive` / `later`；仅 confirmed 进规范库。
9. 宪章、规格、计划、任务、PRD 正文中文优先；命令名与路径保持英文。用户以中文交互时，报告与待办必须中文。
10. 元数据损坏用 `/pm-check`（可 `repair`）。不把 V1.1（全量体检、Git 钩子、单测助手、高危徽标）当成本版本必做。
